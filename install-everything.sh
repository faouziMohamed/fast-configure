...
USAGE_ARGS="[-f|--force] | [-m|--minimal] | [-n|--no-gui] | [-q|--quiet] | [[-e|--exclude] EXCLUDES ] "
...
while [ "${1}" ]; do
  case "${1}" in
    ...
    '-n'|'--no-gui')  IS_SERVER=1; MINIMAL_INSTALL=1;;
    '-q'|'--quiet')   VERBOSE=0;;
    '-e'|'--exclude') 
        shift;
        while [ "${1}" ];do
            case "${1}" in
                ...
                -*) if [[ "${#EXCLUDE_UNEXPECTED_ARGS}" != 0 ]];then EXCLUDE_NO_ARGS=1;fi;break;;
...
function ask_yes_no(){
  ...
  do
      ...
      case "${yn}" in
          ...
          [nN]|no|NO|No|nO) printf "%s\n" "${MESSAGE_FAIL}"; return 1;;
          *) RE_ASK=1;;
      esac
  done
} >/dev/stdout
...
function exclude_error_handler(){
    ...
    if [[ "${FORCE}" == 0 ]];then
        if ask_yes_no "Continue wihout the exlude flag ?"
        then  printf 'Skipping exlude!'; return 0; 
        else printf "Aborting!\n";exit 1
        fi
    else print_error_output 'Skipping exlude!\n'; return 0;
    fi
  sleep 1
}
...
function exclude_unexpected_args() {
  ...
  print_error_output "The '--exclude' flag expect either one or multiple of the folowing arguments\n"
...
function exclude_no_args() {
  ...
  print_error_output "'--exclude' can be used with the folowing arguments\n"
  print_error_output "br | browsers | gr | graphics | df | dot-files, but got ${BOLD}«${UNEXPECTED}»"
  print_error_output "${RESET}\n"
  exclude_error_handler
}
...
function install_services() {
  local SERVICES=(git wget openssh-server)
  now_installing "${SERVICES[@]}"

  # Requirement for git : latest stable version
  sudo add-apt-repository --yes ppa:git-core/ppa
  sudo apt-get install --yes "${SERVICES[@]}"
}

function minimal_utilities_install(){
  local MINIMAL_UTILITIES=(rsync htop tree xz-utils unzip unrar tmux)
  now_installing "${MINIMAL_UTILITIES[@]}"
  sudo apt-get install --yes "${MINIMAL_UTILITIES[@]}"
...
function some_gui_wget_install() {
  ...
  (tar --extract --verbose --file "${XDM_XZ}"; sudo bash ./install.sh) &
...
function main() {
  ...
  if [[ "${FORCE}" == 0 ]]; then helper_; fi

  if [[ "${MINIMAL_INSTALL}" == 0 || "${IS_SERVER}" == 0 ]]; 
  then
    gui_libs_install
